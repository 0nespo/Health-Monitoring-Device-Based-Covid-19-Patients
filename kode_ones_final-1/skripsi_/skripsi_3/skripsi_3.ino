...
void updateThingSpeak(){
  ...
  HTTPClient http;
  sprintf(thingspeakUpdateQuery, "http://api.thingspeak.com/update?api_key=W7D3WOWITWEZQV4N&field1=%d&field2=%s&field3=%s&field5=%s&field6=%s", percent,  heartRateChar, temperatureChar, latitude, longitude);
...
void notifyButtonPressed(){
  ...
  char thingspeakUpdateQuery[150];
  HTTPClient http;
  sprintf(thingspeakUpdateQuery, "http://api.thingspeak.com/update?api_key=BYLUPYMNDA84SOXG-&field1=%s", timer );
  http.begin(client, thingspeakUpdateQuery);
  http.GET();
  http.end();
}
