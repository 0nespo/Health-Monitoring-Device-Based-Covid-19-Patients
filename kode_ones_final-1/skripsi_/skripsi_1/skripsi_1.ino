...
//Constants
const char* SSID = "skripsi"; //ssid wifi
const char* PASSWORD = "asdfghjkl"; //password wifi
...
SoftwareSerial ss(13, 15); // Sofware Serial Object D7-D8
...
void loop()
{
  ...
  if (count == 500) {
    avgIr = avgIr / count;
    avgIr = avgIr / irOffset; //offset providzzes percent
    avgTemp = avgTemp / count;
    count = 0; 
    avgIr = 0;
    avgTemp = 0;
  }
  temperature = temperature-2;//log
...
void updateThingSpeak(){
  ...
  HTTPClient http;
  sprintf(thingspeakUpdateQuery, "http://api.thingspeak.com/update?api_key=RKPSBJZZCTSJ6A9X&field1=%d&field2=%s&field3=%s&field5=%s&field6=%s", percent,  heartRateChar, temperatureChar, latitude, longitude);
...
void notifyButtonPressed(){
  ...
  char thingspeakUpdateQuery[150];
  HTTPClient http;
  sprintf(thingspeakUpdateQuery, "http://api.thingspeak.com/update?api_key=G5LDGJMY1JI4S41H&field1=%s", timer );
  http.begin(client, thingspeakUpdateQuery);
  http.GET();
  http.end();
}
