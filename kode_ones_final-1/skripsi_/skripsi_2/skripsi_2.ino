...
void updateThingSpeak(){
  ...
  HTTPClient http;
  sprintf(thingspeakUpdateQuery, "http://api.thingspeak.com/update?api_key=U83NHFUPODP87ZH9&field1=%d&field2=%s&field3=%s&field5=%s&field6=%s", percent,  heartRateChar, temperatureChar, latitude, longitude);
...
void notifyButtonPressed(){
  ...
  char thingspeakUpdateQuery[150];
  HTTPClient http;
  sprintf(thingspeakUpdateQuery, "http://api.thingspeak.com/update?api_key=VTU6FFUAV3MQK8BR&field1=%s", timer );
  http.begin(client, thingspeakUpdateQuery);
  http.GET();
  http.end();
}
